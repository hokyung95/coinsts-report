"""
========================================================================================
 [모듈 명]: macd_report_daily/macd_report_daily_pdf.py
 [구현 목적]:
   - 빗썸 거래 원화(KRW) 마켓 코인을 대상으로 200개 60분봉(1시간봉) 및 200개 일봉 데이터 수집
   - 일봉 기준 MACD <= 0 이고, MACD 시그널선이 상승 추세(Signal_now > Signal_prev)인 코인을 포착
   - 포착된 코인당 한 페이지에 60분봉 3단 차트(좌측)와 일봉 3단 차트(우측)를 나란히 배치한 PDF 종합 리포트 생성
   - **저장 위치**: macd_report_daily/report/report_macd_daily_YYYYMMDDHHMMSS.pdf
========================================================================================
"""
...
def fetch_and_analyze_single_coin(m, count=200):
    ...
    포착 조건:
    1) 일봉 MACD <= 0
    2) 일봉 MACD Signal 상승 추세 (last_daily_signal > prev_daily_signal)
    """
    ...
    try:
        ...
        if res_daily.status_code == 200:
            ...
            if isinstance(data_daily, list) and len(data_daily) >= 30:
                ...
                macd_daily = last_daily['MACD']
                signal_daily = last_daily['MACD_Signal']
                signal_daily_prev = prev_daily['MACD_Signal']
                
                # 일봉 포착 조건: MACD <= 0 AND Signal 상승 추세 (signal_daily > signal_daily_prev)
                if not pd.isna(macd_daily) and macd_daily <= 0 and not pd.isna(signal_daily) and not pd.isna(signal_daily_prev) and signal_daily > signal_daily_prev:
                    ...
                    if res_1h.status_code == 200:
                        ...
                        if isinstance(data_1h, list) and len(data_1h) >= 30:
                            ...
                            signal_slope = signal_daily - signal_daily_prev
                            ...
                            return {
                                ...
                                'macd_daily': round(macd_daily, 2),
                                'signal_daily': round(signal_daily, 2),
...
def generate_macd_daily_pdf_report(pdf_path=None, max_workers=8):
    """
    일봉 MACD <= 0 & Signal 상승 추세 포착 코인 대상 듀얼 3단 시각화 PDF 리포트 생성
    - PDF 저장 위치: macd_report_daily/report/report_macd_daily_YYYYMMDDHHMMSS.pdf
    """
    ...
    if pdf_path is None:
        ...
        filename = f"report_macd_daily_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
    ...
    print(f"[1/3] 빗썸 {len(markets)}개 원화 코인 스캔 중 (조건: 일봉 MACD <= 0 AND Signal 상승추세, 스레드: {max_workers})...")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if res:
                ...
                print(f" ★ 포착 코인: {res['korean_name']}({res['market']}) | 현재가: {res['close_price']:,}원 | 일봉MACD: {res['macd_daily']:,} | Signal: {res['signal_daily']:,} (기울기: +{res['signal_slope']})")

    # 정렬: 일봉 시그널선 상승 기울기(signal_slope) 내림차순 정렬
    captured_list.sort(key=lambda x: x['signal_slope'], reverse=True)
    
    print(f"\n[2/3] 총 {len(captured_list)}개 포착 코인 PDF 생성 중... (저장경로: {pdf_path})")
    
    if not captured_list:
        print("조건(일봉 MACD <= 0 AND Signal 상승추세)을 만족하는 코인이 없습니다.")
        return None
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        col_labels = ['마켓코드', '한글명', '영문명', '현재가(원)', '일봉 MACD', '일봉 Signal', 'Signal기울기', '일봉 RSI', '60m MACD', '60m Signal', '60m RSI']
        ...
        for page_idx in range(0, len(summary_rows_all), chunk_size):
            ...
            title_text = "빗썸 일봉 MACD (MACD <= 0 & Signal 상승추세) 종합 리포트"
            subtitle_text = f"분석 일시: {now_str} | 조건: 일봉 MACD <= 0 AND MACD Signal 상승 추세 | 총 {len(captured_list)}개 코인 포착 (p.{page_idx//chunk_size + 1})"
        ...
        for idx, item in enumerate(captured_list, 1):
            ...
            title_page = f"[{idx}/{len(captured_list)}] {k_name} ({e_name} / {m_code})  |  현재가: {price:,}원  |  [포착] 일봉 MACD({item['macd_daily']}) <= 0 & Signal 상승추세"
    ...
    print(f"\n[3/3] 성공! 일봉 MACD <= 0 & Signal 상승추세 코인 듀얼 3단 시각화 PDF 리포트 생성 완료!")
