"""
========================================================================================
 [모듈 명]: total_daily_report/total_daily_report_pdf.py
 [구현 목적]:
   - 빗썸 거래 전체 원화(KRW) 마켓 코인을 대상으로 200개의 일봉 데이터를 조회
   - 일목균형표(전환선 9, 기준선 26), MACD(12,26,9), RSI(14) 지표를 계산
   - 코인당 한 페이지로 일목균형표(진한 거래량 포함), MACD, RSI 3단 시각화 PDF 리포트 자동 생성
   - **저장 위치**: total_daily_report/report/report_total_daily_YYYYMMDDHHMMSS.pdf
========================================================================================
"""
...
def fetch_single_coin_daily_200(m):
    """
    단일 코인의 200개 일봉 시계열 데이터 수집 및 지표 분석
    """
    ...
    try:
        ...
        if res_daily.status_code == 200:
            ...
            if isinstance(data_daily, list) and len(data_daily) > 0:
...
def generate_total_daily_pdf_report(pdf_path=None, max_workers=8):
    """
    빗썸 전체 원화 코인 대상 200일봉 일목, MACD, RSI 3단 시각화 PDF 리포트 생성 (1코인 1페이지)
    - PDF 저장 위치: total_daily_report/report/report_total_daily_YYYYMMDDHHMMSS.pdf
    """
    ...
    if pdf_path is None:
        ...
        filename = f"report_total_daily_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
    ...
    print(f"[1/3] 빗썸 {len(markets)}개 원화 거래 코인 전체 200일봉 데이터 수집 중 (스레드: {max_workers})...")
    
    analyzed_list = []
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(fetch_single_coin_daily_200, m): m for m in markets}
    ...
    print(f"\n[2/3] 총 {len(analyzed_list)}개 코인 전체 200일봉 PDF 생성 중... (저장경로: {pdf_path})")
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        col_labels = ['마켓코드', '한글명', '영문명', '현재가(원)', '일봉전환선(9)', '일봉기준선(26)', 'MACD(12,26)', 'RSI(14)']
        ...
        for page_idx in range(0, len(summary_rows_all), chunk_size):
            ...
            title_text = "빗썸 전체 원화 거래 코인 200일봉 종합 리포트"
        ...
        for idx, item in enumerate(analyzed_list, 1):
            ...
            title_str = f"[{idx}/{len(analyzed_list)}] {k_name} ({e_name} / {m_code}) - 200일봉 일목균형표 & 거래량 | 현재가: {price:,}원 | 전환선: {item['conversion_line']:,}원 | 기준선: {item['base_line']:,}원"
    ...
    print(f"\n[3/3] 성공! 빗썸 전체 코인 200일봉 3단 시각화 PDF 리포트 생성 완료!")
    print(f" -> PDF 저장 위치: '{full_pdf_path}'")
    return full_pdf_path

if __name__ == "__main__":
    generate_total_daily_pdf_report()
