"""
========================================================================================
 [모듈 명]: total_onehour_report/total_onehour_report_pdf.py
 [구현 목적]:
   - 빗썸 거래 전체 원화(KRW) 마켓 코인을 대상으로 200개 60분봉(1시간봉) 및 200개 일봉 데이터 수집
   - 60분봉 및 일봉 각각 일목균형표(전환선 9, 기준선 26), MACD(12,26,9), RSI(14) 지표 계산
   - 코인당 한 페이지에 60분봉 3단 차트(좌측)와 일봉 3단 차트(우측)를 나란히 배치한 PDF 종합 리포트 생성
   - **저장 위치**: total_onehour_report/report/report_total_onehour_YYYYMMDDHHMMSS.pdf
========================================================================================
"""
...
def fetch_single_coin_total_onehour(m, count=200):
    """
    ...
    try:
        ...
        if res_1h.status_code == 200 and res_daily.status_code == 200:
            data_1h = res_1h.json()
            data_daily = res_daily.json()
            
            if isinstance(data_1h, list) and len(data_1h) > 0 and isinstance(data_daily, list) and len(data_daily) > 0:
...
def generate_total_onehour_pdf_report(pdf_path=None, max_workers=8):
    """
    빗썸 전체 원화 코인 대상 1시간봉 & 일봉 멀티타임프레임 듀얼 3단 시각화 PDF 리포트 생성 (1코인 1페이지)
    - PDF 저장 위치: total_onehour_report/report/report_total_onehour_YYYYMMDDHHMMSS.pdf
    """
    ...
    if pdf_path is None:
        ...
        filename = f"report_total_onehour_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
    ...
    print(f"[1/3] 빗썸 {len(markets)}개 원화 거래 코인 전체 1시간봉 & 일봉 데이터 수집 중 (스레드: {max_workers})...")
    
    analyzed_list = []
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(fetch_single_coin_total_onehour, m): m for m in markets}
    ...
    print(f"\n[2/3] 총 {len(analyzed_list)}개 코인 전체 1시간봉 & 일봉 PDF 생성 중... (저장경로: {pdf_path})")
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        col_labels = ['마켓코드', '한글명', '영문명', '현재가(원)', '60m전환선', '60m기준선', '60m RSI', '60m MACD', '일봉전환선', '일봉기준선', '일봉 RSI', '일봉 MACD']
        ...
        for page_idx in range(0, len(summary_rows_all), chunk_size):
            ...
            title_text = "빗썸 전체 원화 거래 코인 1시간봉 & 일봉 멀티타임프레임 종합 리포트"
        ...
        for idx, item in enumerate(analyzed_list, 1):
            ...
            title_page = f"[{idx}/{len(analyzed_list)}] {k_name} ({e_name} / {m_code})  |  현재가: {price:,}원  |  1시간봉 & 일봉 멀티타임프레임 시각화"
    ...
    print(f"\n[3/3] 성공! 빗썸 전체 코인 1시간봉 & 일봉 멀티타임프레임 듀얼 3단 시각화 PDF 리포트 생성 완료!")
