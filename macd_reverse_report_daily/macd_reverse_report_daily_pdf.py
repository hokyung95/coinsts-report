"""
========================================================================================
 [모듈 명]: macd_reverse_report_daily/macd_reverse_report_daily_pdf.py
 [구현 목적]:
   - 빗썸 거래 원화(KRW) 마켓 코인을 대상으로 200개 60분봉(1시간봉) 및 200개 일봉 데이터 수집
   - 일봉 기준 종가 가격(Close Price) 기울기가 마이너스(하락)이지만, MACD선 및 Signal선이 평행 또는 우상향(상승/평행)인 코인을 포착
   - 포착된 코인당 한 페이지에 60분봉 3단 차트(좌측)와 일봉 3단 차트(우측)를 나란히 배치한 PDF 종합 리포트 생성
   - **저장 위치**: macd_reverse_report_daily/report/report_macd_reverse_daily_YYYYMMDDHHMMSS.pdf
========================================================================================
"""
...
def fetch_and_analyze_single_coin(m, count=200):
    ...
    포착 조건:
    1) 일봉 종가 가격 기울기 < 0 (Close_now < Close_prev)
    2) 일봉 MACD 기울기 >= 0 (MACD_now >= MACD_prev)
    3) 일봉 Signal 기울기 >= 0 (Signal_now >= Signal_prev)
    """
    ...
    try:
        ...
        if res_daily.status_code == 200:
            ...
            if isinstance(data_daily, list) and len(data_daily) >= 30:
                ...
                price_now = last_daily['Close']
                price_prev = prev_daily['Close']
                price_slope = price_now - price_prev
                price_pct = ((price_now - price_prev) / price_prev) * 100
                
                macd_now = last_daily['MACD']
                macd_prev = prev_daily['MACD']
                macd_slope = macd_now - macd_prev
                
                signal_now = last_daily['MACD_Signal']
                signal_prev = prev_daily['MACD_Signal']
                signal_slope = signal_now - signal_prev
                
                # 포착 조건: 가격 하락(price_slope < 0) AND (MACD slope >= 0) AND (Signal slope >= 0)
                if not pd.isna(price_slope) and price_slope < 0 and not pd.isna(macd_slope) and macd_slope >= 0 and not pd.isna(signal_slope) and signal_slope >= 0:
                    ...
                    if res_1h.status_code == 200:
                        ...
                        if isinstance(data_1h, list) and len(data_1h) >= 30:
                            ...
                            return {
                                ...
                                'macd_daily': round(macd_now, 2),
                                'macd_slope': round(macd_slope, 4),
                                'signal_daily': round(signal_now, 2),
                                'signal_slope': round(signal_slope, 4),
                                ...
                                'signal_1h': round(last_1h['MACD_Signal'], 2) if not pd.isna(last_1h['MACD_Signal']) else 0.0,
...
def generate_macd_reverse_daily_pdf_report(pdf_path=None, max_workers=8):
    """
    일봉 가격 하락 & MACD/Signal선 상승/평행 다이버전스 포착 코인 대상 듀얼 3단 시각화 PDF 리포트 생성
    - PDF 저장 위치: macd_reverse_report_daily/report/report_macd_reverse_daily_YYYYMMDDHHMMSS.pdf
    """
    ...
    if pdf_path is None:
        ...
        filename = f"report_macd_reverse_daily_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
    ...
    print(f"[1/3] 빗썸 {len(markets)}개 원화 코인 스캔 중 (조건: 일봉 가격하락 & MACD/Signal 상승/평행, 스레드: {max_workers})...")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if res:
                ...
                print(f" ★ 포착 코인: {res['korean_name']}({res['market']}) | 현재가: {res['close_price']:,}원({res['price_pct']}%) | MACD기울기: +{res['macd_slope']} | Signal기울기: +{res['signal_slope']}")

    # 정렬: MACD 상승 기울기(macd_slope) + Signal 상승 기울기(signal_slope) 합계 내림차순 정렬
    captured_list.sort(key=lambda x: (x['macd_slope'] + x['signal_slope']), reverse=True)
    
    print(f"\n[2/3] 총 {len(captured_list)}개 포착 코인 PDF 생성 중... (저장경로: {pdf_path})")
    
    if not analyzed_list if 'analyzed_list' in locals() else not captured_list:
        print("조건(일봉 가격하락 & MACD/Signal 상승/평행)을 만족하는 코인이 없습니다.")
        return None
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        col_labels = ['마켓코드', '한글명', '영문명', '현재가(원)', '일봉변동률', '일봉 MACD', 'MACD기울기', '일봉 Signal', 'Signal기울기', '일봉 RSI', '60m MACD', '60m RSI']
        ...
        for page_idx in range(0, len(summary_rows_all), chunk_size):
            ...
            title_text = "빗썸 일봉 MACD 다이버전스 (가격하락 & MACD/Signal 우상향·평행) 리포트"
            subtitle_text = f"분석 일시: {now_str} | 조건: 일봉 종가 기울기 < 0 AND MACD 기울기 >= 0 AND Signal 기울기 >= 0 | 총 {len(captured_list)}개 코인 포착 (p.{page_idx//chunk_size + 1})"
        ...
        for idx, item in enumerate(captured_list, 1):
            ...
            title_page = f"[{idx}/{len(captured_list)}] {k_name} ({e_name} / {m_code})  |  현재가: {price:,}원 ({item['price_pct']}%)  |  [다이버전스] 일봉 가격하락 & MACD/Signal 우상향·평행"
    ...
    print(f"\n[3/3] 성공! 일봉 가격하락 & MACD/Signal 우상향·평행 다이버전스 코인 듀얼 3단 시각화 PDF 리포트 생성 완료!")
    print(f" -> PDF 저장 위치: '{full_pdf_path}'")
    return full_pdf_path

if __name__ == "__main__":
    generate_macd_reverse_daily_pdf_report()
