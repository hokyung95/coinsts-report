"""
========================================================================================
 [모듈 명]: onehour_report/onehour_rsi60_75_simulation_report.py
 [구현 목적]:
   ...
   - 포지션 보유 중 스탑로스(-3.0%) 및 일목균형표 전환선(9) < 기준선(26) 하향 돌파 매도 시뮬레이션
   - 시뮬레이션 매도 후 거래별 수익률, 승률, 손익비(Profit Factor), 평균 수익률 등 수익률 분석 및 3단 차트 포함 PDF 리포트 생성
   - **저장 위치**: onehour_report/report/onehour_rsi60_75_simulation_report_YYYYMMDDHHMMSS.pdf
========================================================================================
"""
...
def generate_onehour_rsi60_75_simulation_pdf_report(pdf_path=None, max_workers=8, stop_loss_pct=-3.0):
    """
    RSI 60~75 (80%+) 매수 & 일목 데드크로스/스탑로스 매도 시뮬레이션 수행 및 PDF 성과 리포트 생성
    - PDF 저장 위치: onehour_report/report/onehour_rsi60_75_simulation_report_YYYYMMDDHHMMSS.pdf
    """
    ...
    if pdf_path is None:
        ...
        filename = f"onehour_rsi60_75_simulation_report_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
    ...
    print(f" -> 조건: 20개 1시간봉 중 60<=RSI<=75 비율 80%+ 매수 & 전환선<기준선 데드크로스 / 스탑로스({stop_loss_pct:+.1f}%) 매도")
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        ax_sum.text(0.5, 0.96, f"빗썸 1시간봉 RSI 60~75 (80%+) & 일목 데드크로스 / 스탑로스({stop_loss_pct:+.1f}%) 시뮬레이션 리포트",
        ...
        for idx, item in enumerate(coin_results[:15], 1):
            ...
            title_page = f"[{idx}/{len(coin_results[:15])}] {k_name} ({m_code}) 시뮬레이션 차트  |  총 {item['total_trades']}회 거래 (승률: {item['win_rate']}%)  |  평균 수익률: {item['avg_return']:+.2f}%"
...
if __name__ == "__main__":
    generate_onehour_rsi60_75_simulation_pdf_report()
