"""
========================================================================================
 [모듈 명]: onehour_report/onehour_bullish60_lower_report.py
 [구현 목적]:
   - 빗썸 거래 원화(KRW) 마켓 코인을 대상으로 200개 60분봉(1시간봉) 및 200개 일봉 데이터 수집
   - 현재 1시간봉이 체결가(종가) > 전환선(9) > 기준선(26) 정배열 상태이고,
   - 1시간봉 기준선(26)의 기울기 각도가 0° 이상 5° 이하(수평 ~ 완만한 상승)이며,
   - 최근 10시간(10개 60분봉) 중 정배열 조건 만족 봉 개수가 6개 이하(<= 6봉)인 초기 정배열 포착 코인을 탐색
   - 포착된 코인당 한 페이지에 60분봉 3단 차트(좌측)와 일봉 3단 차트(우측)를 나란히 배치한 PDF 종합 리포트 생성
   - **저장 위치**: onehour_report/report/onehour_bullish60_lower_report_YYYYMMDDHHMMSS.pdf
========================================================================================
"""
...
def fetch_and_analyze_single_coin(m, count=200):
    ...
    포착 조건:
    1) 현재 60분봉 체결가(Close) > 전환선(9) > 기준선(26) 정배열
    2) 60분봉 기준선(26) 변동 각도가 0° 이상 5° 이하 (0° <= angle <= 5.0°)
    3) 최근 10개 60분봉 중 정배열 만족 봉 수 <= 6개 (1 <= bullish_count <= 6)
    """
    ...
    try:
        ...
        if res_1h.status_code == 200:
            ...
            if isinstance(data_1h, list) and len(data_1h) >= 30:
                ...
                k_now = last_1h['BaseLine']
                k_prev = prev_1h['BaseLine']
                
                # 1) 현재 정배열 검증
                is_current_bullish = not pd.isna(c_now) and not pd.isna(t_now) and not pd.isna(k_now) and (c_now > t_now > k_now)
                
                if is_current_bullish:
                    # 2) 기준선 각도 계산 (0° <= angle <= 5.0°)
                    kijun_diff = k_now - k_prev if not pd.isna(k_prev) else 0.0
                    kijun_pct = (kijun_diff / k_prev) * 100.0 if k_prev > 0 else 0.0
                    angle_deg = np.degrees(np.arctan(max(0.0, kijun_pct)))
                    
                    # 3) 최근 10시간 중 정배열 카운트 (<= 6개)
                    bullish_count = 0
                    for idx in range(-10, 0):
                        c = df_1h['Close'].iloc[idx]
                        t = df_1h['ConversionLine'].iloc[idx]
                        k = df_1h['BaseLine'].iloc[idx]
                        if not pd.isna(c) and not pd.isna(t) and not pd.isna(k) and (c > t > k):
                            bullish_count += 1
                    
                    if (0.0 <= angle_deg <= 5.0) and (1 <= bullish_count <= 6):
                        ...
                        if res_daily.status_code == 200:
                            ...
                            if isinstance(data_daily, list) and len(data_daily) >= 30:
                                ...
                                diff_pct_1h = ((c_now - k_now) / k_now) * 100.0
...
def generate_onehour_bullish60_lower_pdf_report(pdf_path=None, max_workers=8):
    """
    1시간봉 정배열 & 기준선 각도 0°~5° & 최근 10시간 중 정배열 6개 이하 초기 정배열 포착 코인 대상 듀얼 3단 시각화 PDF 리포트 생성
    - PDF 저장 위치: onehour_report/report/onehour_bullish60_lower_report_YYYYMMDDHHMMSS.pdf
    """
    ...
    if pdf_path is None:
        ...
        filename = f"onehour_bullish60_lower_report_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
    ...
    print(f"[1/3] 빗썸 {len(markets)}개 원화 코인 스캔 중 (조건: 정배열 & 60m기준선각도 0°~5° & 10h 중 정배열 <=6개, 스레드: {max_workers})...")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if res:
                ...
                print(f" ★ 포착 코인: {res['korean_name']}({res['market']}) | 현재가: {res['close_price']:,}원 | 기준선각도: {res['angle_deg']}° | 10h 정배열: {res['bullish_count']}/10봉 | 60m이격: +{res['diff_pct_1h']}%")

    # 정렬: 기준선 각도(angle_deg) 오름차순 (수평에 가까운 서열), 동일시 60분봉 이격률 내림차순
    captured_list.sort(key=lambda x: (x['angle_deg'], -x['diff_pct_1h']))
    
    print(f"\n[2/3] 총 {len(captured_list)}개 포착 코인 PDF 생성 중... (저장경로: {pdf_path})")
    
    if not captured_list:
        print("조건(정배열 & 기준선 각도 0°~5° & 10시간 중 정배열 6개 이하)을 만족하는 코인이 없습니다.")
        return None
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        col_labels = ['마켓코드', '한글명', '영문명', '현재가(원)', '기준선각도', '10h 정배열', '60m전환선', '60m기준선', '60m이격률', '60m RSI', '60m MACD', '일봉 RSI']
        ...
        for page_idx in range(0, len(summary_rows_all), chunk_size):
            ...
            title_text = "빗썸 1시간봉 정배열 & 기준선 각도 (0°~5°) & 10h 중 정배열 <= 6봉 리포트"
            subtitle_text = f"분석 일시: {now_str} | 조건: 현재 정배열 AND 0° <= 기준선 각도 <= 5° AND 10시간 중 정배열 <= 6봉 | 총 {len(captured_list)}개 코인 포착 (p.{page_idx//chunk_size + 1})"
        ...
        for idx, item in enumerate(captured_list, 1):
            ...
            title_page = f"[{idx}/{len(captured_list)}] {k_name} ({e_name} / {m_code})  |  현재가: {price:,}원  |  [포착] 정배열 & 기준선각도: {item['angle_deg']}° & 10h중 정배열 {item['bullish_count']}/10봉"
    ...
    print(f"\n[3/3] 성공! 1시간봉 정배열 & 기준선 각도 0°~5° & 10h 정배열 <= 6개 포착 코인 듀얼 3단 시각화 PDF 리포트 생성 완료!")
