"""
========================================================================================
 [모듈 명]: onehour_report/onehour_bullish50_report.py
 [구현 목적]:
   - 빗썸 거래 원화(KRW) 마켓 코인을 대상으로 200개 60분봉(1시간봉) 및 200개 일봉 데이터 수집
   - 최근 10시간(10개 60분봉) 중 체결가(종가) > 전환선(9) > 기준선(26) 정배열 조건을 충족하는 비율이 50% 이상(5개 봉 이상)인 코인을 포착
   - 포착된 코인당 한 페이지에 60분봉 3단 차트(좌측)와 일봉 3단 차트(우측)를 나란히 배치한 PDF 종합 리포트 생성
   - **저장 위치**: onehour_report/report/onehour_bullish50_report_YYYYMMDDHHMMSS.pdf
========================================================================================
"""
...
def fetch_and_analyze_single_coin(m, count=200):
    ...
    포착 조건:
    - 60분봉 기준 최근 10개 캔들 중 체결가(종가) > 전환선(9) > 기준선(26) 정배열 비율 50% 이상 (5개 봉 이상)
    """
    ...
    try:
        ...
        if res_1h.status_code == 200:
            ...
            if isinstance(data_1h, list) and len(data_1h) >= 30:
                ...
                # 최근 10시간 중 체결가 > 전환선 > 기준선 충족 봉 개수 카운트
                bullish_count = 0
                ...
                bullish_ratio = (bullish_count / 10.0) * 100.0
                ...
                if bullish_count >= 5:
                    ...
                    if res_daily.status_code == 200:
                        ...
                        if isinstance(data_daily, list) and len(data_daily) >= 30:
                            ...
                            diff_pct_1h = ((last_1h['Close'] - last_1h['BaseLine']) / last_1h['BaseLine']) * 100
...
def generate_onehour_bullish50_pdf_report(pdf_path=None, max_workers=8):
    """
    1시간봉 최근 10시간 중 체결가 > 전환선 > 기준선 정배열 비율 50% 이상 포착 코인 대상 듀얼 3단 시각화 PDF 리포트 생성
    - PDF 저장 위치: onehour_report/report/onehour_bullish50_report_YYYYMMDDHHMMSS.pdf
    """
    ...
    if pdf_path is None:
        ...
        filename = f"onehour_bullish50_report_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
    ...
    print(f"[1/3] 빗썸 {len(markets)}개 원화 코인 스캔 중 (조건: 최근 10시간 중 1시간봉 체결가 > 전환선 > 기준선 비율 >= 50%, 스레드: {max_workers})...")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if res:
                ...
                print(f" ★ 포착 코인: {res['korean_name']}({res['market']}) | 현재가: {res['close_price']:,}원 | 정배열비율: {res['bullish_ratio']}%({res['bullish_count']}/10봉) | 60m이격: +{res['diff_pct_1h']}%")

    # 정렬: 정배열 비율(bullish_count) 내림차순, 동일 비율 시 이격률(diff_pct_1h) 내림차순 정렬
    captured_list.sort(key=lambda x: (x['bullish_count'], x['diff_pct_1h']), reverse=True)
    
    print(f"\n[2/3] 총 {len(captured_list)}개 포착 코인 PDF 생성 중... (저장경로: {pdf_path})")
    
    if not captured_list:
        print("조건(최근 10시간 중 정배열 비율 50% 이상)을 만족하는 코인이 없습니다.")
        return None
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        summary_rows_all = [
            [
                ...
                c['english_name'],
                f"{c['close_price']:,}",
                f"{c['bullish_ratio']}% ({c['bullish_count']}/10봉)",
                f"{c['tenkan_1h']:,}",
                f"{c['kijun_1h']:,}",
                f"+{c['diff_pct_1h']}%",
                f"{c['rsi_1h']:.1f}",
        ...
        col_labels = ['마켓코드', '한글명', '영문명', '현재가(원)', '10h 정배열비율', '60m전환선', '60m기준선', '60m이격률', '60m RSI', '60m MACD', '일봉전환선', '일봉 RSI']
        ...
        for page_idx in range(0, len(summary_rows_all), chunk_size):
            ...
            title_text = "빗썸 1시간봉 최근 10시간 중 정배열 비율 50% 이상 리포트"
            subtitle_text = f"분석 일시: {now_str} | 조건: 최근 10시간 중 (체결가 > 전환선(9) > 기준선(26)) 충족 5봉(50%) 이상 | 총 {len(captured_list)}개 코인 포착 (p.{page_idx//chunk_size + 1})"
        ...
        for idx, item in enumerate(captured_list, 1):
            ...
            title_page = f"[{idx}/{len(captured_list)}] {k_name} ({e_name} / {m_code})  |  현재가: {price:,}원  |  [포착] 10시간 중 정배열 {item['bullish_ratio']}% ({item['bullish_count']}/10봉)"
    ...
    print(f"\n[3/3] 성공! 1시간봉 10시간 중 정배열 50% 이상 포착 코인 듀얼 3단 시각화 PDF 리포트 생성 완료!")
    print(f" -> PDF 저장 위치: '{full_pdf_path}'")
    return full_pdf_path

if __name__ == "__main__":
    generate_onehour_bullish50_pdf_report()
