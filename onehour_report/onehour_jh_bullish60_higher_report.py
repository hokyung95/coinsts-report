"""
========================================================================================
 [모듈 명]: onehour_report/onehour_jh_bullish60_higher_report.py
 [구현 목적]:
   - 빗썸 거래 원화(KRW) 마켓 코인을 대상으로 200개 60분봉(1시간봉) 및 200개 일봉 데이터 수집
   - 오직 1시간봉 기준 체결가(종가)와 일목균형표 전환선(9)만 체크하여,
   ...
   - **저장 위치**: onehour_report/report/onehour_jh_bullish60_higher_report_YYYYMMDDHHMMSS.pdf
========================================================================================
"""
...
def generate_onehour_jh_bullish60_higher_pdf_report(pdf_path=None, max_workers=8):
    """
    1시간봉 체결가 > 전환선 분포도 >= 60% & 전환선 각도 0°~5° 포착 코인 대상 듀얼 3단 시각화 PDF 리포트 생성
    - PDF 저장 위치: onehour_report/report/onehour_jh_bullish60_higher_report_YYYYMMDDHHMMSS.pdf
    """
    ...
    if pdf_path is None:
        ...
        filename = f"onehour_jh_bullish60_higher_report_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
    ...
    print(f"[1/3] 빗썸 {len(markets)}개 원화 코인 스캔 중 (조건: 60m 체결가>전환선 분포도 >=60% & 전환선각도 0°~5°, 스레드: {max_workers})...")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if res:
                ...
                print(f" ★ 포착 코인: {res['korean_name']}({res['market']}) | 현재가: {res['close_price']:,}원 | 전환선각도: {res['tenkan_angle_deg']}° | 체결가>전환선: {res['tenkan_above_ratio']}%({res['tenkan_above_count']}/10봉)")
    ...
    if not captured_list:
        print("조건(체결가>전환선 분포도 >= 60% AND 전환선 각도 0°~5°)을 만족하는 코인이 없습니다.")
        return None
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        col_labels = ['마켓코드', '한글명', '영문명', '현재가(원)', '10h 체결가>전환선', '전환선각도', '60m전환선', '60m기준선', '전환선이격률', '60m RSI', '60m MACD', '일봉 RSI']
        ...
        for page_idx in range(0, len(summary_rows_all), chunk_size):
            ...
            title_text = "빗썸 1시간봉 체결가>전환선(9) 분포도 >= 60% & 전환선 각도 (0°~5°) 리포트"
            subtitle_text = f"분석 일시: {now_str} | 조건: 최근 10시간 중 체결가>전환선 >= 6봉(60%+) AND 0° <= 전환선 각도 <= 5° | 총 {len(captured_list)}개 코인 포착 (p.{page_idx//chunk_size + 1})"
        ...
        for idx, item in enumerate(captured_list, 1):
            ...
            title_page = f"[{idx}/{len(captured_list)}] {k_name} ({e_name} / {m_code})  |  현재가: {price:,}원  |  [포착] 체결가>전환선 {item['tenkan_above_ratio']}% ({item['tenkan_above_count']}/10봉) & 전환선각도: {item['tenkan_angle_deg']}°"
    ...
    print(f"\n[3/3] 성공! 1시간봉 체결가>전환선 분포도 >= 60% & 전환선 각도 0°~5° 포착 코인 듀얼 3단 시각화 PDF 리포트 생성 완료!")
    print(f" -> PDF 저장 위치: '{full_pdf_path}'")
    return full_pdf_path

if __name__ == "__main__":
    generate_onehour_jh_bullish60_higher_pdf_report()
