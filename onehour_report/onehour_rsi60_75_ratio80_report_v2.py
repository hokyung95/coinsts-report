"""
========================================================================================
 [모듈 명]: onehour_report/onehour_rsi60_75_ratio80_report_v2.py
 [구현 목적]:
   ...
   - **저장 위치**: onehour_report/report/onehour_rsi60_75_ratio80_report_v2_YYYYMMDDHHMMSS.pdf
========================================================================================
"""
...
def fetch_and_analyze_single_coin(m, count=200):
    """
    단일 코인의 60분봉 및 일봉 시계열 데이터 수집 및 분석 (v2)
    포착 조건:
...
def generate_onehour_rsi60_75_ratio80_pdf_report_v2(pdf_path=None, max_workers=8):
    """
    1시간봉 기준 최근 20개봉 중 RSI(14) 60~75 구간 비율 80% 이상(16개 이상) 포착 코인 대상 PDF 리포트 생성 (v2)
    - PDF 저장 위치: onehour_report/report/onehour_rsi60_75_ratio80_report_v2_YYYYMMDDHHMMSS.pdf
    """
    ...
    if pdf_path is None:
        ...
        filename = f"onehour_rsi60_75_ratio80_report_v2_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
    ...
    print(f"[1/3] 빗썸 {len(markets)}개 원화 코인 스캔 중 (조건 v2: 1시간봉 최근 20개봉 중 60<=RSI<=75 비율 80%+ (>=16봉), 스레드: {max_workers})...")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if res:
                ...
                print(f" ★ 포착 코인: {res['korean_name']}({res['market']}) | 현재가: {res['close_price']:,}원 | 20h RSI(60~75): {res['rsi_60_75_ratio']}%({res['rsi_60_75_count']}/20봉) | 상태: {state_str}({res['tenkan_diff_pct']}%) | 60m RSI: {res['rsi_1h']}")
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        col_labels = ['마켓코드', '한글명', '영문명', '현재가(원)', '20h RSI(60~75)', '전환선위치', '전환선이격률', '60m RSI', '60m전환선', '60m기준선', '일봉 RSI', '일봉전환선']
        ...
        for page_idx in range(0, len(summary_rows_all), chunk_size):
            ...
            title_text = "빗썸 1시간봉 최근 20개봉 RSI (60~75) 비율 80% 이상 포착 리포트 (v2)"
        ...
        for idx, item in enumerate(captured_list[:25], 1):
            ...
            title_page = f"[{idx}/{len(captured_list[:25])}] {k_name} ({e_name} / {m_code})  |  현재가: {price:,}원  |  [포착] 20h RSI(60~75): {item['rsi_60_75_ratio']}% ({item['rsi_60_75_count']}/20봉) | 상태: {state_text} ({item['tenkan_diff_pct']:+.2f}%)"
    ...
    print(f"\n[3/3] 성공! 1시간봉 최근 20개봉 RSI(60~75) 80%+ 포착 코인 v2 PDF 리포트 생성 완료!")
