"""
========================================================================================
 [모듈 명]: onehour_report/onehour_rsi60_73_simulation_report_v4.py
 [구현 목적]:
   - 빗썸 거래 전체 원화(KRW) 마켓 코인을 대상으로 최근 200개 60분봉(1시간봉) 수집
   - [매수 조건 v4]: N번째 봉 기준 최근 12개봉(N-11 ~ N)의 RSI(14) 값이 60 초과 73 미만(60 < RSI < 73) 조건 만족 봉 수가 10개 이상(83.3%+)일 때 매수
   - [매도 조건 v4 (OR)]:
   ...
   - **저장 위치**: onehour_report/report/onehour_rsi60_73_simulation_report_v4_YYYYMMDDHHMMSS.pdf
========================================================================================
"""
...
def simulate_single_coin_strategy_v4(m, count=200, stop_loss_pct=-3.0):
    """
    단일 코인의 200개 60분봉 데이터를 수집하고 시뮬레이션 매수/매도 수행 (v4)
    
    [시뮬레이션 규칙]:
    1. 매수 (Buy - v4):
       - N번째 봉 기준 최근 12개봉 (N-11 ~ N) 중 60.0 < RSI < 73.0 범위 봉 수 >= 10개 (83.3% 이상)
    ...
    try:
        ...
        if res_1h.status_code == 200:
            ...
            if isinstance(data_1h, list) and len(data_1h) >= 50:
                ...
                # N번째 봉 기반 시뮬레이션 (최소 12개 봉의 RSI가 확보된 12번째 인덱스부터 시작)
                for idx in range(12, len(df_1h)):
                    row = df_1h.iloc[idx]
                    
                    # 1) 매수 조건 검사 (미보유 상태: 최근 12봉 중 60<RSI<73 10봉 이상)
                    if position is None:
                        window_rsi = df_1h['RSI'].iloc[idx-11 : idx+1]
                        valid_cnt = int(((window_rsi > 60.0) & (window_rsi < 73.0)).sum())
                        
                        if valid_cnt >= 10:  # 83.3% 이상 만족
                            buy_price = row['Close']
                            ...
                            position = {
                                ...
                                'buy_price': buy_price,
                                'buy_time': buy_time,
                                'rsi_ratio': round((valid_cnt / 12.0) * 100.0, 1),
                                'rsi_cnt': valid_cnt
                            }
...
def plot_simulation_chart(ax_p, ax_m, ax_r, df, trades, title_prefix):
    ...
    ax_p.set_title(f"{title_prefix} - [60분봉 12봉 RSI v4 매수/매도 이력 차트 (종가<전환선 OR 손절-3%)]", fontsize=10, fontweight='bold', color='#1b4f72', pad=4)
    ...
    # ---------------------------------------------------------
    # 3단: RSI (14) (60~73 하이라이트 v4)
    # ---------------------------------------------------------
    ax_r.plot(x, df['RSI'], label='RSI(14)', color='#8e44ad', linewidth=1.2)
    ax_r.axhspan(60, 73, color='#f39c12', alpha=0.25, label='60~73 목표구간')
    ax_r.axhline(73, color='#e31a1c', linestyle='--', linewidth=0.8, label='73선')
...
def generate_onehour_rsi60_73_simulation_pdf_report_v4(pdf_path=None, max_workers=8, stop_loss_pct=-3.0):
    """
    최근 12봉 RSI 60<RSI<73 10봉+(83.3%+) 매수 & 종가 < 전환선9 / 손절(-3%) 매도 시뮬레이션 v4 수행 및 PDF 성과 리포트 생성 (손실 정보 포함)
    """
    ...
    if pdf_path is None:
        ...
        filename = f"onehour_rsi60_73_simulation_report_v4_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
    ...
    print(f"[1/3] 빗썸 {len(markets)}개 원화 코인 백테스팅/시뮬레이션 v4 진행 중 (12봉 RSI 60~73 83.3%+ 매수, 매도: 종가<전환선9 OR 손절{stop_loss_pct:+.1f}%)...")
    
    coin_results = []
    all_trades = []
    
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(simulate_single_coin_strategy_v4, m, 200, stop_loss_pct): m for m in markets}
    ...
    print(f"\n[2/3] v4 시뮬레이션 성과 및 손실 정보 분석 PDF 리포트 생성 중... (저장경로: {pdf_path})")
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        ax_sum.text(0.5, 0.96, f"빗썸 1시간봉 12봉 RSI 60~73 (v4) 시뮬레이션 리포트 (수익 & 손실 종합 분석)",
        ...
        # 요약 메트릭 박스 렌더링
        metrics_text = (
            f" [v4 전략 종합 성과 및 손실 메트릭 Summary]\n"
            f" ---------------------------------------------------------\n"
            f" - 매수 조건 v4      : 최근 12개 1시간봉 중 60.0 < RSI < 73.0 만족 봉 >= 10개 (83.3%+)\n"
            f" - 매도 조건 v4 (OR) : 종가 < 일목 전환선(9) OR 스탑로스(-3.0%)\n"
        ...
        # 표 2: 하위 손실 코인 표 (Bottom 7 - 손실 세부 정보 추가!)
        loss_rows = [
            [
                c['market'],
                c['korean_name'],
        ...
        if all_trades_gain:
            ...
            ax_g.axis('off')
            ax_g.text(0.5, 0.96, "v4 시뮬레이션 개별 거래 상세 내역 (▲ 수익 상위 거래 Top 22)", fontsize=14, fontweight='bold', ha='center', va='top', color='#1e8449')
        ...
        if all_trades_loss:
            ...
            ax_l.axis('off')
            ax_l.text(0.5, 0.96, "v4 시뮬레이션 개별 거래 상세 내역 (▼ 최대 손실 거래 Bottom 22 - 손실 원인 분석)", fontsize=14, fontweight='bold', ha='center', va='top', color='#c0392b')
        ...
        for idx, item in enumerate(coin_results_gain[:15], 1):
            ...
            title_page = f"[{idx}/{len(coin_results_gain[:15])}] {k_name} ({m_code}) v4 시뮬레이션 차트  |  총 {item['total_trades']}회 거래 (승률: {item['win_rate']}%)  |  평균 수익률: {item['avg_return']:+.2f}%"
    ...
    print(f"\n[3/3] 성공! v4 시뮬레이션 성과 및 손실 정보 포함 PDF 리포트 생성 완료!")
    print(f" -> PDF 저장 위치: '{full_pdf_path}'")
    return full_pdf_path

if __name__ == "__main__":
    generate_onehour_rsi60_73_simulation_pdf_report_v4()
