"""
========================================================================================
 [모듈 명]: onehour_report/onehour_rsi60_70_ratio80_report.py
 [구현 목적]:
   - 빗썸 거래 전체 원화(KRW) 마켓 코인을 대상으로 200개 60분봉(1시간봉) 및 200개 일봉 데이터 수집
   - 1시간봉 기준 최근 20개봉 중 RSI(14) 값이 60 이상 70 이하(60 <= RSI <= 70) 조건에 해당하는 봉의 비율이 80% 이상 (20개 중 16개 봉 이상)인 코인을 포착
   - 포착된 코인당 한 페이지에 60분봉 3단 차트(좌측)와 일봉 3단 차트(우측)를 나란히 배치한 PDF 종합 리포트 생성
   - **저장 위치**: onehour_report/report/onehour_rsi60_70_ratio80_report_YYYYMMDDHHMMSS.pdf
========================================================================================
"""
...
def fetch_and_analyze_single_coin(m, count=200):
    ...
    포착 조건:
    1시간봉 기준 최근 20개봉 중 60 <= RSI(14) <= 70 범위 조건 충족 봉 80% 이상 (>= 16개 봉)
    """
    ...
    try:
        ...
        if res_1h.status_code == 200:
            ...
            if isinstance(data_1h, list) and len(data_1h) >= 30:
                ...
                # 최근 20개봉 RSI 검증
                recent_20_rsi = df_1h['RSI'].iloc[-20:]
                valid_rsi_mask = (recent_20_rsi >= 60.0) & (recent_20_rsi <= 70.0)
                rsi_60_70_count = int(valid_rsi_mask.sum())
                rsi_60_70_ratio = (rsi_60_70_count / 20.0) * 100.0
                
                # 조건: 최근 20개봉 중 60 <= RSI <= 70 만족 비율 >= 80% (16개 이상)
                if rsi_60_70_count >= 16:
                    ...
                    if res_daily.status_code == 200:
                        ...
                        if isinstance(data_daily, list) and len(data_daily) >= 30:
                            ...
                            return {
                                ...
                                'close_price': c_now,
                                'rsi_60_70_count': rsi_60_70_count,
                                'rsi_60_70_ratio': round(rsi_60_70_ratio, 1),
...
def plot_3tier_chart(axes_col, df, title_prefix, time_frame_label):
    ...
    # ---------------------------------------------------------
    # 3단: RSI (14) (60~70 영역 음영 하이라이트)
    # ---------------------------------------------------------
    ax_r.plot(x, df['RSI'], label='RSI(14)', color='#8e44ad', linewidth=1.2)
    ax_r.axhspan(60, 70, color='#f39c12', alpha=0.25, label='60~70 목표구간')
    ax_r.axhline(70, color='#e31a1c', linestyle='--', linewidth=0.8, label='과매수(70)')
    ax_r.axhline(60, color='#d35400', linestyle=':', linewidth=0.8, label='목표선(60)')
...
def generate_onehour_rsi60_70_ratio80_pdf_report(pdf_path=None, max_workers=8):
    """
    1시간봉 기준 최근 20개봉 중 RSI(14) 60~70 구간 비율 80% 이상(16개 이상) 포착 코인 대상 PDF 리포트 생성
    - PDF 저장 위치: onehour_report/report/onehour_rsi60_70_ratio80_report_YYYYMMDDHHMMSS.pdf
    """
    ...
    if pdf_path is None:
        ...
        filename = f"onehour_rsi60_70_ratio80_report_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
    ...
    print(f"[1/3] 빗썸 {len(markets)}개 원화 코인 스캔 중 (조건: 1시간봉 최근 20개봉 중 60<=RSI<=70 비율 80%+ (>=16봉), 스레드: {max_workers})...")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if res:
                ...
                print(f" ★ 포착 코인: {res['korean_name']}({res['market']}) | 현재가: {res['close_price']:,}원 | 20h RSI(60~70): {res['rsi_60_70_ratio']}%({res['rsi_60_70_count']}/20봉) | 60m RSI: {res['rsi_1h']}")

    # 정렬: RSI 60~70 충족 봉 수(rsi_60_70_count) 내림차순, 현재 1시간봉 RSI 내림차순 정렬
    captured_list.sort(key=lambda x: (x['rsi_60_70_count'], x['rsi_1h']), reverse=True)
    
    print(f"\n[2/3] 총 {len(captured_list)}개 포착 코인 PDF 생성 중... (저장경로: {pdf_path})")
    
    if not captured_list:
        print("조건(최근 20개 1시간봉 중 60 <= RSI <= 70 비율 >= 80%)을 만족하는 코인이 없습니다.")
        return None
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        summary_rows_all = [
            [
                ...
                c['english_name'],
                f"{c['close_price']:,}",
                f"{c['rsi_60_70_ratio']}% ({c['rsi_60_70_count']}/20봉)",
                f"{c['rsi_1h']:.1f}",
                f"{c['macd_1h']:,}",
                f"{c['tenkan_1h']:,}",
                f"{c['kijun_1h']:,}",
        ...
        col_labels = ['마켓코드', '한글명', '영문명', '현재가(원)', '20h RSI(60~70)', '60m RSI', '60m MACD', '60m전환선', '60m기준선', '일봉 RSI', '일봉 MACD', '일봉전환선']
        ...
        for page_idx in range(0, len(summary_rows_all), chunk_size):
            ...
            title_text = "빗썸 1시간봉 최근 20개봉 RSI (60~70) 비율 80% 이상 포착 리포트"
            subtitle_text = f"분석 일시: {now_str} | 조건: 최근 20개 1시간봉 중 60 <= RSI <= 70 충족 봉 >= 16개 (80%+) | 총 {len(captured_list)}개 코인 포착 (p.{page_idx//chunk_size + 1})"
        ...
        for idx, item in enumerate(captured_list, 1):
            ...
            title_page = f"[{idx}/{len(captured_list)}] {k_name} ({e_name} / {m_code})  |  현재가: {price:,}원  |  [포착] 20h RSI(60~70): {item['rsi_60_70_ratio']}% ({item['rsi_60_70_count']}/20봉) | 60m RSI: {item['rsi_1h']}"
    ...
    print(f"\n[3/3] 성공! 1시간봉 최근 20개봉 RSI(60~70) 80%+ 포착 코인 듀얼 3단 시각화 PDF 리포트 생성 완료!")
    print(f" -> PDF 저장 위치: '{full_pdf_path}'")
    return full_pdf_path

if __name__ == "__main__":
    generate_onehour_rsi60_70_ratio80_pdf_report()
