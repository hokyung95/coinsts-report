# onehour_jh_bullish60_higher_report.py 프로세스 및 기술 문서

## 1. 개요 (Overview)

`onehour_jh_bullish60_higher_report.py` 모듈은 **빗썸(Bithumb) 원화(KRW) 마켓에 상장된 전체 암호화폐**를 대상으로 오직 **1시간봉(60분봉) 기준 체결가(종가)와 일목균형표 전환선(9)**만을 체크하여, **최근 10시간 중 체결가 > 전환선(9) 조건 충족 비율이 60% 이상(6개 봉 이상)**이고 **전환선(9)의 기울기 각도가 0° 이상 5° 이하(수평~완만한 우상향)**인 코인을 포착하고, 각 포착 코인별 **60분봉(1시간봉) 및 일봉(Daily) 기준 일목균형표(전환선9, 기준선26), MACD(12,26,9), RSI(14)** 지표를 계산하여 **코인당 한 페이지 듀얼 3단 시각화(좌측: 1시간봉, 우측: 일봉)** PDF 종합 리포트를 자동 생성하는 프로그램입니다.

생성된 PDF 리포트는 `onehour_report/report/` 디렉터리에 타임스탬프 파일명(`onehour_jh_bullish60_higher_report_YYYYMMDDHHMMSS.pdf`) 형태로 자동 저장됩니다.
...
```
    [ 빗썸 1시간봉 체결가>전환선(9) 분포도 >= 60% & 전환선 각도 0°~5° 포착 규격 ]
┌─────────────────────────────────────────────────────────────────────────────┐
│ 1. API 호출:                                                                │
...
```mermaid
flowchart TD
    A[프로그램 시작: onehour_jh_bullish60_higher_report.py] --> B[빗썸 API로 원화 KRW 전체 마켓 목록 조회]
    B --> C[ThreadPoolExecutor 멀티스레드 스캔 생성 max_workers=8]
    C --> D[각 코인별 1시간봉 수집: 체결가>전환선 분포도 >= 60% & 전환선 각도 0°~5° 검증]
    D -->|조건 만족 코인| E[일봉 추가 수집 및 60m/일봉 일목·MACD·RSI 지표 계산]
    E --> F[수집 데이터 검증 및 체결가>전환선 분포도/각도 정렬]
    F --> G[PdfPages 시작: onehour_report/report/onehour_jh_bullish60_higher_report_YYYYMMDDHHMMSS.pdf]
...
- 마켓코드, 한글명, 영문명, 현재가, 10h 체결가>전환선 비율(%), 전환선 각도(°), 60m 전환선/기준선/이격률/RSI/MACD, 일봉 RSI 요약 표 수록.
...
```
d:\pyprj\coinsts\
├── onehour_jh_bullish60_higher_report.py             # [루트 래퍼] 메인 실행 모듈
└── onehour_report/
    ├── onehour_jh_bullish60_higher_report.py         # [메인 모듈] 체결가>전환선 60%+ & 각도 0°~5° 검증 및 PDF 생성
    ├── onehour_jh_bullish60_higher_report.md         # [기술 문서] 본 문서
    └── report/                                       # [PDF 저장 폴더]
        └── onehour_jh_bullish60_higher_report_YYYYMMDDHHMMSS.pdf
```
...
python onehour_report/onehour_jh_bullish60_higher_report.py

# 2. 루트 래퍼 스크립트 실행
python onehour_jh_bullish60_higher_report.py
```
