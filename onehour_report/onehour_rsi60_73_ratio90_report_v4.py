"""
========================================================================================
 [모듈 명]: onehour_report/onehour_rsi60_73_ratio90_report_v4.py
 [구현 목적]:
   - 빗썸 거래 전체 원화(KRW) 마켓 코인을 대상으로 200개 60분봉(1시간봉) 및 200개 일봉 데이터 수집
   - 1시간봉 기준 최근 12개봉(N-11 ~ N) 중 RSI(14) 값이 60 초과 73 미만(60 < RSI < 73) 조건에 해당하는 봉 수가 10개 이상인 코인을 포착
   - 체결가(Close) vs 일목균형표 전환선(9) 이격도 및 이탈 위치 정보를 포함한 코인별 듀얼 3단 시각화 PDF 리포트 생성
   - **저장 위치**: onehour_report/report/onehour_rsi60_73_ratio90_report_v4_YYYYMMDDHHMMSS.pdf
========================================================================================
"""
...
def fetch_and_analyze_single_coin(m, count=200):
    """
    단일 코인의 60분봉 및 일봉 시계열 데이터 수집 및 분석 (v4)
    포착 조건:
    1시간봉 기준 최근 12개봉 중 60 < RSI(14) < 73 범위 조건 충족 봉 수 >= 10개 (>= 83.3%)
    """
    ...
    try:
        ...
        if res_1h.status_code == 200:
            ...
            if isinstance(data_1h, list) and len(data_1h) >= 30:
                ...
                # 최근 12개봉 RSI 검증 (60 < RSI < 73)
                recent_12_rsi = df_1h['RSI'].iloc[-12:]
                valid_rsi_mask = (recent_12_rsi > 60.0) & (recent_12_rsi < 73.0)
                rsi_60_73_count = int(valid_rsi_mask.sum())
                rsi_60_73_ratio = (rsi_60_73_count / 12.0) * 100.0
                
                # 조건: 최근 12개봉 중 60 < RSI < 73 만족 봉 수 >= 10개
                if rsi_60_73_count >= 10:
                    ...
                    if res_daily.status_code == 200:
                        ...
                        if isinstance(data_daily, list) and len(data_daily) >= 30:
                            ...
                            return {
                                ...
                                'close_price': c_now,
                                'rsi_60_73_count': rsi_60_73_count,
                                'rsi_60_73_ratio': round(rsi_60_73_ratio, 1),
...
def generate_onehour_rsi60_73_ratio90_pdf_report_v4(pdf_path=None, max_workers=8):
    """
    1시간봉 기준 최근 12개봉 중 RSI(14) 60~73 구간 충족 봉 수 10개 이상(>=83.3%) 포착 코인 대상 PDF 리포트 생성 (v4)
    - PDF 저장 위치: onehour_report/report/onehour_rsi60_73_ratio90_report_v4_YYYYMMDDHHMMSS.pdf
    """
    ...
    if pdf_path is None:
        ...
        filename = f"onehour_rsi60_73_ratio90_report_v4_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
    ...
    print(f"[1/3] 빗썸 {len(markets)}개 원화 코인 스캔 중 (조건 v4: 1시간봉 최근 12개봉 중 60<RSI<73 충족 봉 >= 10개, 스레드: {max_workers})...")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if res:
                ...
                print(f" ★ 포착 코인: {res['korean_name']}({res['market']}) | 현재가: {res['close_price']:,}원 | 12h RSI(60~73): {res['rsi_60_73_ratio']}%({res['rsi_60_73_count']}/12봉) | 상태: {state_str}({res['tenkan_diff_pct']}%) | 60m RSI: {res['rsi_1h']}")

    # 정렬: RSI 60~73 충족 봉 수(rsi_60_73_count) 내림차순, 현재 1시간봉 RSI 내림차순 정렬
    captured_list.sort(key=lambda x: (x['rsi_60_73_count'], x['rsi_1h']), reverse=True)
    
    print(f"\n[2/3] 총 {len(captured_list)}개 포착 코인 PDF 생성 중... (저장경로: {pdf_path})")
    
    if not captured_list:
        print("조건(최근 12개 1시간봉 중 60 < RSI < 73 충족 봉 >= 10개)을 만족하는 코인이 없습니다.")
        return None
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        summary_rows_all = [
            [
                ...
                c['english_name'],
                f"{c['close_price']:,}",
                f"{c['rsi_60_73_ratio']}% ({c['rsi_60_73_count']}/12봉)",
        ...
        col_labels = ['마켓코드', '한글명', '영문명', '현재가(원)', '12h RSI(60~73)', '전환선위치', '전환선이격률', '60m RSI', '60m전환선', '60m기준선', '일봉 RSI', '일봉전환선']
        ...
        for page_idx in range(0, len(summary_rows_all), chunk_size):
            ...
            title_text = "빗썸 1시간봉 최근 12개봉 RSI (60~73) 10봉 이상 포착 리포트 (v4)"
            subtitle_text = f"분석 일시: {now_str} | 조건: 최근 12개 1시간봉 중 60 < RSI < 73 충족 봉 >= 10개 (83.3%+) | 총 {len(captured_list)}개 코인 포착 (p.{page_idx//chunk_size + 1})"
        ...
        for idx, item in enumerate(captured_list[:25], 1):
            ...
            title_page = f"[{idx}/{len(captured_list[:25])}] {k_name} ({e_name} / {m_code})  |  현재가: {price:,}원  |  [포착 v4] 12h RSI(60~73): {item['rsi_60_73_ratio']}% ({item['rsi_60_73_count']}/12봉) | 상태: {state_text} ({item['tenkan_diff_pct']:+.2f}%)"
    ...
    print(f"\n[3/3] 성공! 1시간봉 최근 12개봉 RSI(60~73) 10봉+ 포착 코인 v4 PDF 리포트 생성 완료!")
    print(f" -> PDF 저장 위치: '{full_pdf_path}'")
    return full_pdf_path

if __name__ == "__main__":
    generate_onehour_rsi60_73_ratio90_pdf_report_v4()
