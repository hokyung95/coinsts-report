"""
========================================================================================
 [모듈 명]: onehour_report/onehour_rsi60_75_16bars_simulation_report.py
 [구현 목적]:
   - 빗썸 거래 전체 원화(KRW) 마켓 코인을 대상으로 최근 200개 60분봉(1시간봉) 수집
   - [조정된 매수 조건]:
     ① N번째 봉 기준 최근 16개봉(N-15 ~ N)의 RSI(14) 값이 60 이상 75 이하(60 <= RSI <= 75) 조건 만족 봉 수가 13개 이상(80%+)
     ② [직전 3개봉 저RSI 필터]: N번째 봉 기준 직전 3개봉(N-18, N-17, N-16 / 17~19번째 봉)의 RSI(14) 값이 모두 60 이하(<= 60.0)일 것!
   - [매도 조건 (OR)]:
     ① 목표가 익절: 매수체결가 대비 +50.0% 이상 상승 시 익절 매도
     ② 체결가 < 전환선: 거래가격(종가 Close)이 일목 전환선(9)을 하향 이탈할 때 매도
     ③ 스탑로스: 매수체결가 대비 -3.0% 이하 손실 발생 시 손절 매도
   - 시뮬레이션 매도 후 거래별 수익률, 승률, 손익비(Profit Factor), 상위 수익/하위 손실 코인 및 최대 손실 거래 세부 내역 표 포함 PDF 리포트 생성
   - **저장 위치**: onehour_report/report/onehour_rsi60_75_16bars_simulation_report_YYYYMMDDHHMMSS.pdf
========================================================================================
"""
...
def simulate_single_coin_strategy_16bars(m, count=200, take_profit_pct=50.0, stop_loss_pct=-3.0):
    """
    단일 코인의 200개 60분봉 데이터를 수집하고 시뮬레이션 매수/매도 수행 (16봉 기준 + 이전 3개봉 RSI <= 60 필터)
    
    [시뮬레이션 규칙]:
    1. 매수 (Buy):
       - N번째 봉 기준 최근 16개봉 (N-15 ~ N) 중 60 <= RSI <= 75 범위 봉 수 >= 13개 (80% 이상)
       - AND 직전 3개봉 (N-18, N-17, N-16) RSI <= 60.0 모두 만족 시 N번째 봉 종가로 매수
    2. 매도 (Sell OR 조건):
       - [목표가 익절 +50%]: 매수체결가 대비 고가(high_price) 또는 종가가 +50.0% 이상 달성 시 매도
       - [스탑로스 매도 -3%]: 진입가 대비 저가(low_price) 손실률이 stop_loss_pct (-3.0%) 이하 손실 발생 시 매도
    ...
    try:
        ...
        if res_1h.status_code == 200:
            ...
            if isinstance(data_1h, list) and len(data_1h) >= 50:
                ...
                # N번째 봉 기반 시뮬레이션 (최소 19개 봉의 RSI가 확보된 19번째 인덱스부터 시작)
                for idx in range(19, len(df_1h)):
                    row = df_1h.iloc[idx]
                    
                    # 1) 매수 조건 검사 (미보유 상태: 최근 16봉 중 60<=RSI<=75 13봉 이상 AND 직전 3개봉 RSI <= 60.0)
                    if position is None:
                        window_rsi = df_1h['RSI'].iloc[idx-15 : idx+1]
                        valid_cnt = int(((window_rsi >= 60.0) & (window_rsi <= 75.0)).sum())
                        
                        prev_3_rsi = df_1h['RSI'].iloc[idx-18 : idx-15]
                        prev_3_valid = (prev_3_rsi <= 60.0).all()
                        
                        if valid_cnt >= 13 and prev_3_valid:  # 80% 이상 & 이전 3개봉 저RSI 만족
                            buy_price = row['Close']
                            ...
                            position = {
                                ...
                                'buy_price': buy_price,
                                'buy_time': buy_time,
                                'rsi_ratio': round((valid_cnt / 16.0) * 100.0, 1),
                                'rsi_cnt': valid_cnt,
                                'prev_3_max_rsi': round(prev_3_rsi.max(), 1)
                            }
                    
                    # 2) 매도 조건 검사 (보유 상태 OR 조건)
                    else:
                        conv = row['ConversionLine']
                        c_price = row['Close']
                        h_price = row['high_price']
                        l_price = row['low_price']
                        
                        target_tp_price = position['buy_price'] * (1.0 + take_profit_pct / 100.0)
                        target_sl_price = position['buy_price'] * (1.0 + stop_loss_pct / 100.0)
                        
                        is_take_profit = (h_price >= target_tp_price) or (((c_price - position['buy_price']) / position['buy_price'] * 100.0) >= take_profit_pct)
                        is_stop_loss = (l_price <= target_sl_price) or (((c_price - position['buy_price']) / position['buy_price'] * 100.0) <= stop_loss_pct)
                        is_price_cross_below_tenkan = not pd.isna(conv) and (c_price < conv)
                        is_last_candle = (idx == len(df_1h) - 1)
                        
                        if is_take_profit or is_stop_loss or is_price_cross_below_tenkan or is_last_candle:
                            if is_take_profit:
                                sell_price = max(c_price, target_tp_price)
                                sell_reason = f'목표가 익절 (+{take_profit_pct:.1f}%)'
                            elif is_stop_loss:
                            ...
                            trades.append({
                                ...
                                'prev_3_max_rsi': position['prev_3_max_rsi'],
                                'sell_idx': idx,
...
def plot_simulation_chart(ax_p, ax_m, ax_r, df, trades, title_prefix):
    """
    시뮬레이션 거래 매수/매도 시점이 표시된 3단 차트 그리기
    """
    x = range(len(df))
    ...
    ax_p.set_title(f"{title_prefix} - [60분봉 16봉 RSI 80%+ & 직전3봉 RSI<=60 매수/매도 이력 차트]", fontsize=10, fontweight='bold', color='#1b4f72', pad=4)
...
def generate_onehour_rsi60_75_16bars_simulation_pdf_report(pdf_path=None, max_workers=8, take_profit_pct=50.0, stop_loss_pct=-3.0):
    """
    최근 16봉 RSI 60~75 13봉+(80%+) AND 직전 3개봉 RSI <= 60 매수 & 종가 < 전환선9 / 익절(+50%) / 손절(-3%) 매도 시뮬레이션 수행 및 PDF 성과 리포트 생성 (손실 정보 포함)
    - PDF 저장 위치: onehour_report/report/onehour_rsi60_75_16bars_simulation_report_YYYYMMDDHHMMSS.pdf
    """
    ...
    if pdf_path is None:
        ...
        filename = f"onehour_rsi60_75_16bars_simulation_report_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
    ...
    print(f"[1/3] 빗썸 {len(markets)}개 원화 코인 백테스팅/시뮬레이션 진행 중 (16봉 RSI 60~75 80%+ AND 직전 3개봉 RSI<=60 매수, 매도: 종가<전환선9 OR 익절+{take_profit_pct:.1f}% OR 손절{stop_loss_pct:+.1f}%)...")
    
    coin_results = []
    all_trades = []
    
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(simulate_single_coin_strategy_16bars, m, 200, take_profit_pct, stop_loss_pct): m for m in markets}
    ...
    # 정렬: 수익 코인 / 손실 코인 및 최고/최대손실 거래 정렬
    all_trades_gain = sorted([t for t in all_trades if t['return_pct'] >= 0], key=lambda x: x['return_pct'], reverse=True)
    all_trades_loss = sorted([t for t in all_trades if t['return_pct'] < 0], key=lambda x: x['return_pct'])
    
    coin_results_gain = sorted(coin_results, key=lambda x: (x['avg_return'], x['win_rate']), reverse=True)
    coin_results_loss = sorted(coin_results, key=lambda x: (x['avg_return'], x['win_rate']))

    print(f"\n[2/3] 16봉 조율 시뮬레이션 성과 및 손실 정보 분석 PDF 리포트 생성 중... (저장경로: {pdf_path})")
    ...
    # 터미널 콘솔 성과 요약 출력
    print("=" * 80)
    print("      [ 16봉 RSI 60~75 (80%+) & 직전 3봉 RSI<=60 시뮬레이션 성과 종합 결과 ]")
    print("=" * 80)
    ...
    with PdfPages(pdf_path) as pdf:
        # -------------------------------------------------------------
        # 1. 성과 종합 요약 (Executive Summary) 페이지 (수익 코인 Top 7 + 손실 코인 Bottom 7 표)
        # -------------------------------------------------------------
        fig_sum, ax_sum = plt.subplots(figsize=(14, 8.5))
        ax_sum.axis('off')
        
        ax_sum.text(0.5, 0.96, f"빗썸 1시간봉 16봉 RSI 60~75 (80%+) & 직전 3봉 RSI<=60 시뮬레이션 리포트",
                    fontsize=13.5, fontweight='bold', ha='center', va='top', color='#1a5276')
        ax_sum.text(0.5, 0.92, f"분석 일시: {now_str} | 수집 대상: 빗썸 원화 마켓 코인 최근 200개 1시간봉",
                    fontsize=10, color='gray', ha='center', va='top')

        # 요약 메트릭 박스 렌더링
        metrics_text = (
            f" [전략 종합 성과 및 손실 메트릭 Summary]\n"
            f" ---------------------------------------------------------\n"
            f" - 조정된 매수 조건  : 최근 16봉 중 60<=RSI<=75 >= 13봉 (80%+) AND 직전 3개봉(N-18~N-16) RSI <= 60\n"
            f" - 매도 조건 (OR)    : 종가 < 일목 전환선(9) OR 목표가 익절(+50.0%) OR 스탑로스(-3.0%)\n"
            f" - 총 거래 횟수      : {total_trades_count}회 (수익 {len(winning_trades)}회 / 손실 {len(losing_trades)}회)\n"
            f" - 전체 승률         : {overall_win_rate:.1f}%  |  거래당 평균 수익률: {overall_avg_return:+.2f}%\n"
            f" - 손익비 (Profit Factor): {profit_factor:.2f}  |  평균 보유 시간: {avg_holding_bars:.1f}시간\n"
            f" - 최고 수익 거래    : {max_gain:+.2f}%  |  최대 손실 거래: {max_loss:+.2f}%"
        )
        ax_sum.text(0.05, 0.86, metrics_text, fontsize=9.5, fontfamily='Malgun Gothic', va='top',
                    bbox=dict(boxstyle='round,pad=0.7', facecolor='#eaf2f8', edgecolor='#2980b9', alpha=0.9))

        # 표 1: 상위 수익 코인 표 (Top 7)
        gain_rows = [
            [
                c['market'],
                c['korean_name'],
            ...
            ] for c in coin_results_gain[:7]
        ]
        col_labels = ['마켓코드', '한글명', '총 거래횟수', '승률(%)', '평균 수익률(%)', '최대 수익(%)', '최대 손실(%)']
        
        ax_sum.text(0.05, 0.58, "▲ 상위 수익 코인 Top 7 (평균 수익률 기준)", fontsize=10, fontweight='bold', color='#1e8449')
        t_gain = ax_sum.table(cellText=gain_rows, colLabels=col_labels, cellLoc='center', loc='center', bbox=[0.05, 0.35, 0.90, 0.21])
        t_gain.auto_set_font_size(False)
        t_gain.set_fontsize(7.5)
        for col_i in range(len(col_labels)):
            cell = t_gain[(0, col_i)]
            cell.set_facecolor('#1e8449')
            cell.set_text_props(color='white', fontweight='bold')

        # 표 2: 하위 손실 코인 표 (Bottom 7)
        loss_rows = [
            [
                c['market'],
                c['korean_name'],
                ...
                f"{max([t['return_pct'] for t in c['trades']]):+.2f}%",
                f"{min([t['return_pct'] for t in c['trades']]):+.2f}%"
            ] for c in coin_results_loss[:7]
        ]
        ax_sum.text(0.05, 0.30, "▼ 하위 손실 발생 코인 Bottom 7 (최대 손실/손절 기준)", fontsize=10, fontweight='bold', color='#c0392b')
        t_loss = ax_sum.table(cellText=loss_rows, colLabels=col_labels, cellLoc='center', loc='center', bbox=[0.05, 0.07, 0.90, 0.21])
        t_loss.auto_set_font_size(False)
        t_loss.set_fontsize(7.5)
        for col_i in range(len(col_labels)):
            cell = t_loss[(0, col_i)]
        ...
        # -------------------------------------------------------------
        # 2. 수익 거래 및 최대 손실 거래 세부 내역 표 페이지
        # -------------------------------------------------------------
        col_labels_t = ['마켓코드', '한글명', '매수가(원)', '매도가(원)', '수익률(%)', '보유시간', '매수시점', '매도시점', '청산 사유']

        # 2-1) 수익 상위 거래 표 페이지 (Top 22 Trades)
        if all_trades_gain:
            trade_rows_gain = [
                [
                    t['market'],
                    ...
                    t['sell_reason']
                ] for t in all_trades_gain[:22]
            ]
            fig_g, ax_g = plt.subplots(figsize=(14, 8.5))
            ax_g.axis('off')
            ax_g.text(0.5, 0.96, "시뮬레이션 개별 거래 상세 내역 (▲ 수익 상위 거래 Top 22)", fontsize=14, fontweight='bold', ha='center', va='top', color='#1e8449')
            ax_g.text(0.5, 0.92, f"총 {len(winning_trades)}개 수익 거래 중 최고 수익률 상위 거래 내역 표", fontsize=9.5, color='gray', ha='center', va='top')
            
            tg_table = ax_g.table(cellText=trade_rows_gain, colLabels=col_labels_t, cellLoc='center', loc='center', bbox=[0.02, 0.05, 0.96, 0.82])
            tg_table.auto_set_font_size(False)
            tg_table.set_fontsize(8.0)
            for col_i in range(len(col_labels_t)):
                cell = tg_table[(0, col_i)]
        ...
        # 2-2) 최대 손실 거래 표 페이지 (Bottom 22 Losing Trades)
        if all_trades_loss:
            trade_rows_loss = [
                [
                    t['market'],
                    ...
                    t['sell_time'].split('T')[-1][:5] if 'T' in str(t['sell_time']) else str(t['sell_idx'])+'봉',
                    t['sell_reason']
                ] for t in all_trades_loss[:22]
            ]
            fig_l, ax_l = plt.subplots(figsize=(14, 8.5))
            ax_l.axis('off')
            ax_l.text(0.5, 0.96, "시뮬레이션 개별 거래 상세 내역 (▼ 최대 손실 거래 Bottom 22 - 손실 원인 분석)", fontsize=14, fontweight='bold', ha='center', va='top', color='#c0392b')
            ax_l.text(0.5, 0.92, f"총 {len(losing_trades)}개 손실 거래 중 손실률이 가장 컸던 하위 22개 거래 (스탑로스/데드크로스 손실 사유 포함)", fontsize=9.5, color='gray', ha='center', va='top')
            
            tl_table = ax_l.table(cellText=trade_rows_loss, colLabels=col_labels_t, cellLoc='center', loc='center', bbox=[0.02, 0.05, 0.96, 0.82])
            tl_table.auto_set_font_size(False)
            tl_table.set_fontsize(8.0)
            for col_i in range(len(col_labels_t)):
                cell = tl_table[(0, col_i)]
                cell.set_facecolor('#c0392b')
                cell.set_text_props(color='white', fontweight='bold')
            plt.tight_layout()
            pdf.savefig(fig_l)
        ...
        # -------------------------------------------------------------
        for idx, item in enumerate(coin_results_gain[:15], 1):
            ...
            title_page = f"[{idx}/{len(coin_results_gain[:15])}] {k_name} ({m_code}) 16봉 시뮬레이션 차트  |  총 {item['total_trades']}회 거래 (승률: {item['win_rate']}%)  |  평균 수익률: {item['avg_return']:+.2f}%"
    ...
    print(f"\n[3/3] 성공! 16봉 RSI(60~75) 80%+ AND 직전3봉 RSI<=60 시뮬레이션 성과 PDF 리포트 생성 완료!")
    print(f" -> PDF 저장 위치: '{full_pdf_path}'")
    return full_pdf_path

if __name__ == "__main__":
    generate_onehour_rsi60_75_16bars_simulation_pdf_report()
