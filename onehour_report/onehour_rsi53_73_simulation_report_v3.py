"""
========================================================================================
 [모듈 명]: onehour_report/onehour_rsi53_73_simulation_report_v3.py
 [구현 목적]:
   - 빗썸 거래 전체 원화(KRW) 마켓 코인을 대상으로 최근 200개 60분봉(1시간봉) 수집
   - [v3 매수 조건]: N번째 봉 기준 최근 12개봉(N-11 ~ N)의 RSI(14) 값이 53 이상 73 이하(53 <= RSI <= 73) 조건 만족 봉 수가 10개 이상일 때 매수
   - [v3 매도 조건]: 거래가격(종가 Close)이 일목균형표 전환선(9)을 하향 이탈(체결가 < 전환선)할 때 매도
   - [보완 매도 조건]: 스탑로스(-3.0%) 이하 손실 발생 시 손절 매도
   - 시뮬레이션 매도 후 거래별 수익률, 승률, 손익비(Profit Factor), 평균 수익률 등 수익률 분석 및 3단 차트 포함 PDF 리포트 생성
   - **저장 위치**: onehour_report/report/onehour_rsi53_73_simulation_report_v3_YYYYMMDDHHMMSS.pdf
========================================================================================
"""
...
def simulate_single_coin_strategy_v3(m, count=200, stop_loss_pct=-3.0):
    """
    단일 코인의 200개 60분봉 데이터를 수집하고 시뮬레이션 매수/매도 수행 (v3)
    
    [시뮬레이션 규칙 v3]:
    1. 매수 (Buy v3):
       - N번째 봉 기준 최근 12개봉 (N-11 ~ N) 중 53 <= RSI <= 73 범위 봉 수 >= 10개
        ...
        if res_1h.status_code == 200:
            ...
            if isinstance(data_1h, list) and len(data_1h) >= 50:
                ...
                for idx in range(12, len(df_1h)):
                    ...
                    # 1) 매수 조건 검사 (미보유 상태 v3: 최근 12봉 중 53<=RSI<=73 10봉 이상)
                    if position is None:
                        window_rsi = df_1h['RSI'].iloc[idx-11 : idx+1]
                        valid_cnt = int(((window_rsi >= 53.0) & (window_rsi <= 73.0)).sum())
                        
                        if valid_cnt >= 10:  # 10개 봉 이상 만족
                            buy_price = row['Close']
                    ...
                    # 2) 매도 조건 검사 (보유 상태 v3: 체결가 < 전환선 또는 스탑로스)
                    else:
...
def plot_simulation_chart(ax_p, ax_m, ax_r, df, trades, title_prefix):
    """
    시뮬레이션 거래 매수/매도 시점이 표시된 3단 차트 그리기 (v3)
    """
    x = range(len(df))
    ...
    ax_p.set_title(f"{title_prefix} - [60분봉 v3 매수/매도 이력 차트 (12봉 RSI 53~73 10봉+)]", fontsize=10, fontweight='bold', color='#1b4f72', pad=4)
    ...
    ax_r.axhline(53, color='#27ae60', linestyle=':', linewidth=0.8, label='53선')
...
def generate_onehour_rsi53_73_simulation_pdf_report_v3(pdf_path=None, max_workers=8, stop_loss_pct=-3.0):
    """
    최근 12봉 RSI 53~73 10봉+ 매수 & 체결가 < 전환선 / 스탑로스 매도 시뮬레이션 수행 및 PDF 성과 리포트 생성 (v3)
    - PDF 저장 위치: onehour_report/report/onehour_rsi53_73_simulation_report_v3_YYYYMMDDHHMMSS.pdf
    """
    ...
    if pdf_path is None:
        ...
        filename = f"onehour_rsi53_73_simulation_report_v3_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
    ...
    print(f"[1/3] 빗썸 {len(markets)}개 원화 코인 백테스팅/시뮬레이션 진행 중 (v3 - 매수: 12봉 RSI 53~73 10봉+, 매도: 체결가 < 전환선9, 스탑로스: {stop_loss_pct:+.1f}%)...")
    print(f" -> 조건: 최근 12개 1시간봉 중 53<=RSI<=73 >= 10봉 매수 & 체결가<전환선9 / 스탑로스({stop_loss_pct:+.1f}%) 매도")
    
    coin_results = []
    all_trades = []
    
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(simulate_single_coin_strategy_v3, m, 200, stop_loss_pct): m for m in markets}
    ...
    print(f"\n[2/3] v3 시뮬레이션 성과 분석 및 PDF 리포트 생성 중... (저장경로: {pdf_path})")
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        ax_sum.text(0.5, 0.96, f"빗썸 1시간봉 최근 12봉 RSI 53~73 (10봉+) & 체결가<전환선9 시뮬레이션 리포트 (v3)",
        ...
        # 요약 메트릭 박스 렌더링
        metrics_text = (
            f" [v3 전략 종합 성과 메트릭 Summary]\n"
            f" ---------------------------------------------------------\n"
            f" - 신규 매수 조건    : 최근 12개 1시간봉 중 53.0 <= RSI <= 73.0 만족 봉 >= 10개\n"
        ...
        for page_idx in range(0, len(trade_rows_all), chunk_size):
            ...
            ax_t.text(0.5, 0.96, "v3 시뮬레이션 개별 거래 상세 내역 표 (상위 거래)", fontsize=15, fontweight='bold', ha='center', va='top', color='#1a5276')
        ...
        for idx, item in enumerate(coin_results[:15], 1):
            ...
            title_page = f"[{idx}/{len(coin_results[:15])}] {k_name} ({m_code}) v3 시뮬레이션 차트  |  총 {item['total_trades']}회 거래 (승률: {item['win_rate']}%)  |  평균 수익률: {item['avg_return']:+.2f}%"
    ...
    print(f"\n[3/3] 성공! v3 시뮬레이션 성과 분석 및 PDF 리포트 생성 완료!")
    print(f" -> PDF 저장 위치: '{full_pdf_path}'")
    return full_pdf_path

if __name__ == "__main__":
    generate_onehour_rsi53_73_simulation_pdf_report_v3()
