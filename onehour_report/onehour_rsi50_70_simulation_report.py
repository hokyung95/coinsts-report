"""
========================================================================================
 [모듈 명]: onehour_report/onehour_rsi50_70_simulation_report.py
 [구현 목적]:
   - 빗썸 거래 전체 원화(KRW) 마켓 코인을 대상으로 최근 200개 60분봉(1시간봉) 수집
   - N번째 봉 기준 최근 20개봉(N-19 ~ N)의 RSI(14) 값이 50 이상 70 이하(50 <= RSI <= 70) 조건 만족 비율이 80% 이상(16개봉 이상)일 때 매수
   - 포지션 보유 중 일목균형표 전환선(9)이 기준선(26)을 하향 돌파(전환선 < 기준선)할 때 매도 시뮬레이션
   - 시뮬레이션 매도 후 거래별 수익률, 승률, 손익비(Profit Factor), 평균 수익률 등 수익률 분석 및 3단 차트 포함 PDF 리포트 생성
   - **저장 위치**: onehour_report/report/onehour_rsi50_70_simulation_report_YYYYMMDDHHMMSS.pdf
========================================================================================
"""
...
def simulate_single_coin_strategy(m, count=200, stop_loss_pct=-3.0):
    """
    단일 코인의 200개 60분봉 데이터를 수집하고 시뮬레이션 매수/매도 수행
    
    [시뮬레이션 규칙]:
    1. 매수 (Buy):
       - N번째 봉 기준 최근 20개봉 (N-19 ~ N) 중 50 <= RSI <= 70 범위 봉 수 >= 16개 (80% 이상)
       - 미보유 상태에서 매수 조건 충족 시 N번째 봉 종가로 매수
    2. 매도 (Sell):
       - [스탑로스 매도]: 진입가 대비 저가(low_price) 손실률이 stop_loss_pct (예: -3.0%) 이하 손실 발생 시 매도
       - [데드크로스 매도]: 보유 상태에서 전환선(9) < 기준선(26) (하향 돌파/데드크로스) 시 해당 봉 종가로 매도
    ...
    try:
        time.sleep(0.02)
        res_1h = requests.get(url_1h, headers=headers, timeout=4)
            ...
            if isinstance(data_1h, list) and len(data_1h) >= 50:
                ...
                for idx in range(20, len(df_1h)):
                    ...
                    if position is None:
                        ...
                        valid_cnt = int(((window_rsi >= 50.0) & (window_rsi <= 70.0)).sum())
                    ...
                    # 2) 매도 조건 검사 (보유 상태)
                        ...
                        # 스탑로스 기준가격 (buy_price * (1 + stop_loss_pct / 100))
                        target_sl_price = position['buy_price'] * (1.0 + stop_loss_pct / 100.0)
                        
                        is_stop_loss = (row['low_price'] <= target_sl_price) or (((row['Close'] - position['buy_price']) / position['buy_price'] * 100.0) <= stop_loss_pct)
                        is_dead_cross = not pd.isna(conv) and not pd.isna(base) and (conv < base)
                        is_last_candle = (idx == len(df_1h) - 1)
                        
                        if is_stop_loss or is_dead_cross or is_last_candle:
                            if is_stop_loss:
                                sell_price = min(row['Close'], target_sl_price)
                                sell_reason = f'스탑로스 ({stop_loss_pct:+.1f}%)'
                            elif is_dead_cross:
                                sell_price = row['Close']
                                sell_reason = '전환선 < 기준선 데드크로스'
                            else:
                                sell_price = row['Close']
                                sell_reason = '마지막봉 청산'
...
def plot_simulation_chart(ax_p, ax_m, ax_r, df, trades, title_prefix):
    ...
    ax_p.set_title(f"{title_prefix} - [60분봉 시뮬레이션 매수/매도 이력 차트]", fontsize=10, fontweight='bold', color='#1b4f72', pad=4)
    ...
    # ---------------------------------------------------------
    # 3단: RSI (14) (50~70 하이라이트)
    # ---------------------------------------------------------
    ax_r.plot(x, df['RSI'], label='RSI(14)', color='#8e44ad', linewidth=1.2)
    ax_r.axhspan(50, 70, color='#f39c12', alpha=0.25, label='50~70 목표구간')
    ax_r.axhline(70, color='#e31a1c', linestyle='--', linewidth=0.8, label='70선')
    ax_r.axhline(50, color='#27ae60', linestyle=':', linewidth=0.8, label='50선')
...
def generate_onehour_rsi50_70_simulation_pdf_report(pdf_path=None, max_workers=8, stop_loss_pct=-3.0):
    """
    RSI 50~70 (80%+) 매수 & 일목 데드크로스/스탑로스 매도 시뮬레이션 수행 및 PDF 성과 리포트 생성
    - PDF 저장 위치: onehour_report/report/onehour_rsi50_70_simulation_report_YYYYMMDDHHMMSS.pdf
    """
    ...
    if pdf_path is None:
        ...
        filename = f"onehour_rsi50_70_simulation_report_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
    ...
    print(f"[1/3] 빗썸 {len(markets)}개 원화 코인 백테스팅/시뮬레이션 진행 중 (스탑로스: {stop_loss_pct:+.1f}%)...")
    print(f" -> 조건: 20개 1시간봉 중 50<=RSI<=70 비율 80%+ 매수 & 전환선<기준선 데드크로스 / 스탑로스({stop_loss_pct:+.1f}%) 매도")
    
    coin_results = []
    all_trades = []
    
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(simulate_single_coin_strategy, m, 200, stop_loss_pct): m for m in markets}
    ...
    print(f"\n[2/3] 시뮬레이션 성과 분석 및 PDF 리포트 생성 중... (저장경로: {pdf_path})")
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        ax_sum.text(0.5, 0.96, f"빗썸 1시간봉 RSI 50~70 (80%+) & 일목 데드크로스 / 스탑로스({stop_loss_pct:+.1f}%) 시뮬레이션 리포트",
                    fontsize=15, fontweight='bold', ha='center', va='top', color='#1a5276')
        ax_sum.text(0.5, 0.92, f"분석 일시: {now_str} | 수집 대상: 빗썸 원화 마켓 코인 최근 200개 1시간봉",
                    fontsize=10, color='gray', ha='center', va='top')

        # 요약 메트릭 박스 렌더링
        metrics_text = (
            f" [전략 종합 성과 메트릭 Summary]\n"
            f" ---------------------------------------------------------\n"
        ...
        for page_idx in range(0, len(trade_rows_all), chunk_size):
            ...
            ax_t.text(0.5, 0.96, "시뮬레이션 개별 거래 상세 내역 표 (상위 거래)", fontsize=15, fontweight='bold', ha='center', va='top', color='#1a5276')
        ...
        for idx, item in enumerate(coin_results[:15], 1):
            ...
            title_page = f"[{idx}/{len(coin_results)}] {k_name} ({m_code}) 시뮬레이션 차트  |  총 {item['total_trades']}회 거래 (승률: {item['win_rate']}%)  |  평균 수익률: {item['avg_return']:+.2f}%"
    ...
    print(f"\n[3/3] 성공! 시뮬레이션 성과 분석 및 PDF 리포트 생성 완료!")
    print(f" -> PDF 저장 위치: '{full_pdf_path}'")
    return full_pdf_path

if __name__ == "__main__":
    generate_onehour_rsi50_70_simulation_pdf_report()
